...
def carregar_credenciais():
    if not os.path.exists(CREDENTIALS_PATH):
        ...
        print("3. Execute o script novamente (a partir da pasta scripts)")
    ...
    if os.path.exists(TOKEN_PATH):
        ...
        try:
            ...
            print("Iniciando fluxo de autenticação manual...")
            print("IMPORTANTE: Faça login com suporte@careintelligence.ai")
            print("=" * 60)
            
            # Método manual que não precisa de servidor local
            # Definir redirect_uri para uma das URIs configuradas no Google Cloud
            flow.redirect_uri = 'http://localhost:60817'
            auth_url, _ = flow.authorization_url(prompt='consent')
            
            print("\nABRA ESTA URL NO SEU NAVEGADOR:")
            print(f"{auth_url}")
            print("\nApós fazer login, você será redirecionado para uma página")
            print("que pode mostrar erro. COPIE A URL COMPLETA dessa página.")
            print("=" * 60)
            
            # Solicitar o código de autorização
            auth_code = input("\nCole aqui a URL completa da página de redirecionamento: ").strip()
            
            # Extrair o código da URL
            if "code=" in auth_code:
                import urllib.parse as urlparse
                parsed = urlparse.urlparse(auth_code)
                ...
                if code:
                    ...
                    raise Exception("Não foi possível extrair o código da URL")
            else:
                # Talvez o usuário colou apenas o código
                print(f"\nTentando usar como código direto...")
