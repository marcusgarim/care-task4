...
# Bootstrap opcional do PostgreSQL (ignora erro se PG não estiver configurado)
if [ -f "$ROOT_DIR/scripts/bootstrap_pg.py" ]; then
  echo "[dev] Executando bootstrap do PostgreSQL (se configurado)..."
  (cd "$ROOT_DIR" && python scripts/bootstrap_pg.py) || echo "[WARN] Bootstrap PG falhou. Prosseguindo."
fi
