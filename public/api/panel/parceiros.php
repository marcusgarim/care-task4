...
try {
    $stmt = $pdo->query("SHOW COLUMNS FROM parceiros LIKE 'ativo'");
    if ($stmt->rowCount() == 0) {
...
try {
    $stmt = $pdo->query("SHOW COLUMNS FROM parceiros LIKE 'telefone'");
    if ($stmt->rowCount() == 0) {
        $pdo->exec("ALTER TABLE parceiros ADD COLUMN telefone VARCHAR(20) NULL");
    }
} catch (Exception $e) {
    // Ignora erro se a tabela não existir
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        // Buscar todos os parceiros (incluindo inativos para edição)
        $stmt = $pdo->prepare("SELECT * FROM parceiros ORDER BY nome");
        $stmt->execute();
        ...
        if (!$input || !isset($input['nome'])) {
            throw new Exception('Nome do parceiro é obrigatório');
        }
        
        $stmt = $pdo->prepare("
            INSERT INTO parceiros (
                tipo, nome, endereco, telefone, ativo
            ) VALUES (?, ?, ?, ?, ?)
        ");
        ...
        if (count($input) === 2 && isset($input['ativo'])) {
            $stmt = $pdo->prepare("UPDATE parceiros SET ativo = ? WHERE id = ?");
            ...
            $stmt = $pdo->prepare("
                UPDATE parceiros SET 
                    tipo = ?, nome = ?, endereco = ?, telefone = ?, ativo = ?, updated_at = NOW()
                WHERE id = ?
            ");
        ...
        $message = isset($input['ativo']) && $input['ativo'] == 0 ? 
            'Parceiro desativado com sucesso' : 'Parceiro atualizado com sucesso';
